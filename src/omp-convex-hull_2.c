...
void convex_hull(const points_t *pset, points_t *hull)
{
    ...
    int *local_next;
    int ok = 0;
#pragma omp parallel default(none) shared(cur, leftmost, hull, n, local_next, p, ok)
    {
        int const thread_num = omp_get_num_threads();
        int const thread_id = omp_get_thread_num();
#pragma omp single
        {
            local_next = (int*) malloc(thread_num * sizeof(int));
        }
...
#pragma omp single
            ...
            if(ok) {
                local_next[thread_id] = (cur + 1) % n;
                int j;
#pragma omp for
                for(j = 0; j < n; j++) {
                    /* if(LEFT == turn(p[cur], p[local_next[thread_id]], p[j])) {
                            update local_next
                    */
                ...
                //In a parallel for, calculate angle between all the best nodes -> remember initial fake node for "prev"
                /* prev = modified_cur -> out of parallel section
                    max_angle = 0
                    reduction max(max:max_angle)
                    for(j=0 to thread_num) {
                        if(calculate_angle(prev, cur, p[local_next[j]]) > max_angle)
                        {
                            update max_angle
                        }
                    }
...
#pragma omp single
        ...
        //Only one thread initializes local next

        //Do
            //One thread asserts that null->n < n, adds current vertex to hull and increases n
            
            //update localnext[thread_id] -> all localnext are the same at this point
            //parallel for, but next is changed to localnext[thread_id]

            //implicit barrier at the end of for

            //Take 1: serial search -> one thread looks for the leftmost between all localnext and updates cur
            //(Not now) Take 2: in a parallel for, calculate all angles between use min reduction and the angle 
